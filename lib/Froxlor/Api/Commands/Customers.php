...
class Customers extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	{
		if ($this->isAdmin()) {
			$this->logger()->addNotice("[API] list customers");
	...
	public function get()
	{
		...
		if ($result) {
			...
			$this->logger()->addNotice("[API] get customer '" . $result['loginname'] . "'");
		...
		if ($this->isAdmin()) {
			if ($this->getUserDetail('customers_used') < $this->getUserDetail('customers') || $this->getUserDetail('customers') == '-1') {
				...
				if (! \Froxlor\Validate\Validate::validateEmail($email)) {
					...
					$this->logger()->addInfo("[API] added customer '" . $loginname . "'");
					unset($ins_data);
					...
					$this->logger()->addNotice("[API] automatically added " . $stats_folder . " htpasswd for user '" . $loginname . "'");
					Database::pexecute($ins_stmt, $ins_data, true, true);
					...
					if ($createstdsubdomain == '1') {
						...
						try {
							...
							$this->logger()->addError("[API] Unable to add standard-subdomain: " . $e->getMessage());
						}
						...
						if ($domainid > 0) {
							...
							$this->logger()->addNotice("[API] automatically added standardsubdomain for user '" . $loginname . "'");
							\Froxlor\System\Cronjob::inserttask('1');
						}
					}
					...
					if ($sendpassword == '1') {
						...
						if ($_mailerror) {
							$this->logger()->addError("[API] Error sending mail: " . $mailerr_msg);
							\Froxlor\UI\Response::standard_error('errorsendingmail', $email, true);
						}

						$this->mailer()->clearAddresses();
						$this->logger()->addNotice("[API] automatically sent password to user '" . $loginname . "'");
					}
				}
				$this->logger()->addWarning("[API] added customer '" . $loginname . "'");
	...
	public function update()
	{
		...
		if ($this->isAdmin()) {
			...
			if ($createstdsubdomain == '1' && $result['standardsubdomain'] == '0') {
				...
				try {
					...
					$this->logger()->addWarning("[API] Unable to add standard-subdomain: " . $e->getMessage());
				}
				...
				if ($domainid > 0) {
					...
					$this->logger()->addNotice("[API] automatically added standardsubdomain for user '" . $result['loginname'] . "'");
					\Froxlor\System\Cronjob::inserttask('1');
				}
			}
			...
			if ($createstdsubdomain == '0' && $result['standardsubdomain'] != '0') {
				try {
					...
					$this->logger()->addError("[API] Unable to delete standard-subdomain: " . $e->getMessage());
				}
				$this->logger()->addNotice("[API] automatically deleted standardsubdomain for user '" . $result['loginname'] . "'");
				\Froxlor\System\Cronjob::inserttask('1');
			}
			...
			if ($deactivated != $result['deactivated']) {
				...
				$this->logger()->addInfo("[API] " . ($deactivated ? 'deactivated' : 'reactivated') . " user '" . $result['loginname'] . "'");
				\Froxlor\System\Cronjob::inserttask('1');
			}
		...
		$this->logger()->addInfo("[API] edited user '" . $result['loginname'] . "'");
	...
	public function delete()
	{
		if ($this->isAdmin()) {
			...
			$this->logger()->addWarning("[API] deleted customer '" . $result['loginname'] . "'");
			return $this->response(200, "successfull", $result);
		}
		throw new \Exception("Not allowed to execute given command.", 403);
	}
	...
	public function unlock()
	{
		if ($this->isAdmin()) {
			...
			$this->logger()->addWarning("[API] unlocked customer '" . $result['loginname'] . "'");
			return $this->response(200, "successfull", $result);
		}
		throw new \Exception("Not allowed to execute given command.", 403);
	}
	...
	public function move()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			$this->logger()->addInfo("[API] moved user '" . $c_result['loginname'] . "' from admin/reseller '" . $c_result['adminname'] . " to admin/reseller '" . $a_result['loginname'] . "'");
