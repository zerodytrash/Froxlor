...
class Domains extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function get()
	{
		if ($this->isAdmin()) {
			...
			if ($result) {
				$this->logger()->addNotice("[API] get domain '" . $result['domain'] . "'");
		...
		if ($this->isAdmin()) {
			if ($this->getUserDetail('domains_used') < $this->getUserDetail('domains') || $this->getUserDetail('domains') == '-1') {
				...
				if ($domain == '') {
					...
					$this->logger()->addWarning("[API] added domain '" . $domain . "'");
	...
	public function update()
	{
		if ($this->isAdmin()) {
			...
			if ($isemaildomain == '0' && $result['isemaildomain'] == '1') {
				...
				$this->logger()->addNotice("[API] deleted domain #" . $id . " from mail-tables as is-email-domain was set to 0");
			}
			...
			if ($ssfs == 1) {
				...
				$this->logger()->addInfo("[API] removed specialsettings on all subdomains of domain #" . $id);
			}
	...
	public function delete()
	{
		if ($this->isAdmin()) {
			...
			if ($idString != '') {
				...
				Database::pexecute($del_stmt, $paramString, true, true);
				$this->logger()->addNotice("[API] deleted domain/s from mail-tables");
			}
			...
			$this->logger()->addInfo("[API] deleted domain/subdomains (#" . $result['id'] . ")");
