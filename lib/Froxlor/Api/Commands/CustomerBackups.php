...
class CustomerBackups extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		...
		$this->logger()->addNotice("[API] added customer-backup job for '" . $customer['loginname'] . "'. Target directory: " . $userpath);
	...
	public function listing()
	{
		...
		$this->logger()->addNotice("[API] list customer-backups");
		...
		if ($result['count'] > 0 && $entry > 0) {
			...
			foreach ($result['list'] as $backupjob) {
				if ($backupjob['id'] == $entry && in_array($backupjob['data']['customerid'], $customer_ids)) {
					...
					$this->logger()->addNotice("[API] deleted planned customer-backup #" . $entry);
