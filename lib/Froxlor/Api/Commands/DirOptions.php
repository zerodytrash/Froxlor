...
class DirOptions extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		...
		$this->logger()->addInfo("[API] added directory-option for '" . $userpath . "'");
	...
	public function get()
	{
		...
		if ($result) {
			$this->logger()->addNotice("[API] get directory options for '" . $result['path'] . "'");
			return $this->response(200, "successfull", $result);
		}
		$key = "id #" . $id;
	...
	public function update()
	{
		...
		if (($options_indexes != $result['options_indexes']) || ($error404path != $result['error404path']) || ($error403path != $result['error403path']) || ($error500path != $result['error500path']) || ($options_cgi != $result['options_cgi'])) {
			...
			Database::pexecute($stmt, $params, true, true);
			$this->logger()->addInfo("[API] edited directory options for '" . str_replace($customer['documentroot'], '/', $result['path']) . "'");
		}
	...
	public function listing()
	{
		...
		$this->logger()->addNotice("[API] list directory-options");
	...
	public function delete()
	{
		...
		if ((int) Settings::Get('perl.suexecworkaround') == 1) {
			...
			if (file_exists($perlsymlink)) {
				...
				$this->logger()->addDebug("[API] deleted suexecworkaround symlink '" . $perlsymlink . "'");
			}
			// remove folder in suexec-path
			if (file_exists($suexecpath)) {
				\Froxlor\FileDir::safe_exec('rm -rf ' . escapeshellarg($suexecpath));
				$this->logger()->addDebug("[API] deleted suexecworkaround path '" . $suexecpath . "'");
			}
		}
		$stmt = Database::prepare("
		...
		$this->logger()->addInfo("[API] deleted directory-option for '" . str_replace($customer_data['documentroot'], '/', $result['path']) . "'");
		\Froxlor\System\Cronjob::inserttask('1');
		return $this->response(200, "successfull", $result);
	}
