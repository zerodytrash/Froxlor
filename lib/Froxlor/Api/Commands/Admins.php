...
class Admins extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function listing()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			$this->logger()->addNotice("[API] list admins");
	...
	public function get()
	{
		...
		if ($this->isAdmin() && ($this->getUserDetail('change_serversettings') == 1 || ($this->getUserDetail('adminid') == $id || $this->getUserDetail('loginname') == $loginname))) {
			...
			if ($result) {
				$this->logger()->addNotice("[API] get admin '" . $result['loginname'] . "'");
	...
	public function add()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			if (strtolower($loginname_check['loginname']) == strtolower($loginname) || strtolower($loginname_check_admin['loginname']) == strtolower($loginname)) {
				...
				$this->logger()->addWarning("[API] added admin '" . $loginname . "'");
		...
		if ($this->isAdmin()) {
			...
			if ($this->getUserDetail('change_serversettings') == 1 || $result['adminid'] == $this->getUserDetail('adminid')) {
				...
				if (! \Froxlor\Validate\Validate::validateEmail($email)) {
					...
					$this->logger()->addInfo("[API] edited admin '" . $result['loginname'] . "'");
	...
	public function delete()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			$this->logger()->addWarning("[API] deleted admin '" . $result['loginname'] . "'");
	...
	public function unlock()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			$this->logger()->addWarning("[API] unlocked admin '" . $result['loginname'] . "'");
			return $this->response(200, "successfull", $result);
		}
		throw new \Exception("Not allowed to execute given command.", 403);
	}
