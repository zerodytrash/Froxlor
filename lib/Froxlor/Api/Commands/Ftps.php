...
class Ftps extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		...
		if ($this->getUserDetail('ftps_used') < $this->getUserDetail('ftps') || $this->getUserDetail('ftps') == '-1') {
			...
			if (! empty($username_check) && $username_check['username'] = $username) {
				...
				$this->logger()->addWarning("[API] added ftp-user '" . $username . "'");
	...
	public function get()
	{
		...
		if ($result) {
			$this->logger()->addNotice("[API] get ftp-user '" . $result['username'] . "'");
			return $this->response(200, "successfull", $result);
		}
	...
	public function update()
	{
		...
		if ($password != '') {
			...
			$this->logger()->addInfo("[API] updated ftp-account password for '" . $result['username'] . "'");
		}
		...
		if ($path != '') {
			...
			if ($path != $result['homedir']) {
				...
				$this->logger()->addInfo("[API] updated ftp-account homdir for '" . $result['username'] . "'");
			}
		}
		...
		$this->logger()->addNotice("[API] updated ftp-user '" . $result['username'] . "'");
		return $this->response(200, "successfull", $result);
	}
	...
	public function listing()
	{
		...
		$this->logger()->addNotice("[API] list ftp-users");
