...
class Certificates extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		...
		if (empty($result)) {
			...
			$this->logger()->addInfo("[API] added ssl-certificate for '" . $domain['domain'] . "'");
			$result = $this->apiCall('Certificates.get', array(
				'id' => $domain['id']
			));
	...
	public function get()
	{
		...
		$this->logger()->addInfo("[API] get ssl-certificate for '" . $domain['domain'] . "'");
	...
	public function update()
	{
		...
		$this->logger()->addInfo("[API] updated ssl-certificate for '" . $domain['domain'] . "'");
		$result = $this->apiCall('Certificates.get', array(
			'id' => $domain['id']
		));
		return $this->response(200, "successfull", $result);
	 ...
	 * @access admin, customer
	 * @throws \Exception
	 * @return string json-encoded array count|list|countall
	 */
