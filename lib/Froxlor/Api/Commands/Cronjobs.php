...
class Cronjobs extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function update()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			$this->logger()->addInfo("[API] cronjob with description '" . $result['module'] . '/' . $result['cronfile'] . "' has been updated by '" . $this->getUserDetail('loginname') . "'");
			$result = $this->apiCall('Cronjobs.get', array(
				'id' => $id
			));
			return $this->response(200, "successfull", $result);
	...
	{
		if ($this->isAdmin()) {
			$this->logger()->addNotice("[API] list cronjobs");
			$result_stmt = Database::prepare("
