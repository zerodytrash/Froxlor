...
class PhpSettings extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	{
		if ($this->isAdmin()) {
			$this->logger()->addNotice("[API] list php-configs");
	...
	public function add()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			$this->logger()->addInfo("[API] php setting with description '" . $description . "' has been created by '" . $this->getUserDetail('loginname') . "'");
	...
	public function update()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			\Froxlor\System\Cronjob::inserttask('1');
			$this->logger()->addInfo("[API] php setting with description '" . $description . "' has been updated by '" . $this->getUserDetail('loginname') . "'");
	...
	public function delete()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings') == 1) {
			...
			\Froxlor\System\Cronjob::inserttask('1');
			$this->logger()->addInfo("[API] php setting '" . $result['description'] . "' has been deleted by '" . $this->getUserDetail('loginname') . "'");
			return $this->response(200, "successfull", $result);
		}
		throw new \Exception("Not allowed to execute given command.", 403);
	}
