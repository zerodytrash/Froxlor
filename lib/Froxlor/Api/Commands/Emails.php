...
class Emails extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		...
		if ($this->getUserDetail('emails_used') < $this->getUserDetail('emails') || $this->getUserDetail('emails') == '-1') {
			...
			$this->logger()->addInfo("[API] added email address '" . $email_full . "'");
	...
	public function get()
	{
		...
		if ($result) {
			$this->logger()->addNotice("[API] get email address '" . $result['email_full'] . "'");
			return $this->response(200, "successfull", $result);
		}
	...
	public function update()
	{
		...
		Database::pexecute($stmt, $params, true, true);
		$this->logger()->addInfo("[API] toggled catchall-flag for email address '" . $result['email_full'] . "'");
	...
	public function listing()
	{
		...
		$this->logger()->addNotice("[API] list email-addresses");
	...
	public function delete()
	{
		...
		$this->logger()->addInfo("[API] deleted email address '" . $result['email_full'] . "'");
		return $this->response(200, "successfull", $result);
	}
}
