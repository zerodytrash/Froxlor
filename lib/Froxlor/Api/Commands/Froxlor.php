...
class Froxlor extends \Froxlor\Api\ApiCommand
{
	...
	public function importSettings()
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings')) {
			...
			$this->logger()->addNotice("User " . $this->getUserDetail('loginname') . " imported settings");
			try {
	...
	{
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings')) {
			$this->logger()->addNotice("User " . $this->getUserDetail('loginname') . " exported settings");
	...
	public function updateSetting()
	{
		...
		if ($this->isAdmin() && $this->getUserDetail('change_serversettings')) {
			...
			$this->logger()->addWarning("[API] Changing setting '" . $setting . "' from '" . $oldvalue . "' to '" . $value . "'");
