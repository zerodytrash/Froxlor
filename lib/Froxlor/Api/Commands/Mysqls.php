...
class Mysqls extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		if ($this->getUserDetail('mysqls_used') < $this->getUserDetail('mysqls') || $this->getUserDetail('mysqls') == '-1') {
			...
			$this->logger()->addWarning("[API] added mysql-database '" . $username . "'");
	...
	public function update()
	{
		...
		$this->logger()->addWarning("[API] updated mysql-database '" . $result['databasename'] . "'");
		$result = $this->apiCall('Mysqls.get', array(
	...
	public function delete()
	{
		...
		$this->logger()->addWarning("[API] deleted database '" . $result['databasename'] . "'");
		return $this->response(200, "successfull", $result);
	}
}
