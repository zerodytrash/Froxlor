...
class DirProtections extends \Froxlor\Api\ApiCommand implements \Froxlor\Api\ResourceEntity
{
	...
	public function add()
	{
		...
		$this->logger()->addInfo("[API] added directory-protection for '" . $username . " (" . $path . ")'");
	...
	public function get()
	{
		...
		if ($result) {
			$this->logger()->addNotice("[API] get directory protection for '" . $result['path'] . "'");
			return $this->response(200, "successfull", $result);
		}
	...
	public function update()
	{
		...
		$this->logger()->addInfo("[API] updated directory-protection '" . $result['username'] . " (" . $result['path'] . ")'");
		$result = $this->apiCall('DirProtections.get', array(
			'id' => $result['id']
		));
		return $this->response(200, "successfull", $result);
	...
	public function listing()
	{
		...
		$this->logger()->addNotice("[API] list directory-protections");
	...
	public function delete()
	{
		...
		$this->logger()->addInfo("[API] deleted htpasswd for '" . $result['username'] . " (" . $result['path'] . ")'");
		\Froxlor\System\Cronjob::inserttask('1');
		return $this->response(200, "successfull", $result);
	}
}
