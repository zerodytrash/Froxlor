...
class IntegrityCheck
{
	...
	public function databaseCharset($fix = false)
	{
		...
		if (! empty($charset) && strtolower($charset) != 'utf8') {
			$this->log->addNotice("database charset seems to be different from UTF-8, integrity-check can fix that");
			if ($fix) {
				// fix database
				...
				$this->log->addWarning("database charset was different from UTF-8, integrity-check fixed that");
			} else {
				return false;
			}
		}
		...
		while ($row = $result_stmt->fetch(\PDO::FETCH_ASSOC)) {
			if (! array_key_exists($row['id_ipandports'], $ips)) {
				if ($fix) {
					...
					$this->log->addWarning("found an ip/port-id in domain <> ip table which does not exist, integrity check fixed this");
				} else {
					$this->log->addNotice("found an ip/port-id in domain <> ip table which does not exist, integrity check can fix this");
					return false;
				}
			}
			if (! array_key_exists($row['id_domain'], $domains)) {
				if ($fix) {
					...
					$this->log->addWarning("found a domain-id in domain <> ip table which does not exist, integrity check fixed this");
				} else {
					$this->log->addNotice("found a domain-id in domain <> ip table which does not exist, integrity check can fix this");
					return false;
				}
			}
		...
		foreach ($domains as $domainid => $adminid) {
			if (! array_key_exists($domainid, $ipstodomains)) {
				if ($fix) {
					...
					$this->log->addWarning("found a domain-id with no entry in domain <> ip table, integrity check fixed this");
				} else {
					$this->log->addNotice("found a domain-id with no entry in domain <> ip table, integrity check can fix this");
					return false;
				}
			}
		}
	...
	public function subdomainSslRedirect($fix = false)
	{
		...
		foreach ($parentdomains as $id => $sslavailable) {
			...
			if ($fix) {
				...
				$this->log->addWarning("found a subdomain with ssl_redirect=1 but parent-domain has ssl=0, integrity check fixed this");
			} else {
				// It's just the check, let the function fail
				$this->log->addNotice("found a subdomain with ssl_redirect=1 but parent-domain has ssl=0, integrity check can fix this");
				return false;
			}
		}
	...
	public function subdomainLetsencrypt($fix = false)
	{
		...
		foreach ($parentdomains as $id => $sslavailable) {
			...
			if ($fix) {
				...
				$this->log->addWarning("found a subdomain with letsencrypt=1 but parent-domain has ssl=0, integrity check fixed this");
			} else {
				// It's just the check, let the function fail
				$this->log->addNotice("found a subdomain with letsencrypt=1 but parent-domain has ssl=0, integrity check can fix this");
				return false;
			}
		}
	...
	public function webserverGroupMemberForFcgidPhpFpm($fix = false)
	{
		...
		if ($cwg_stmt->rowCount() > 0) {
			$this->log->addNotice("Customers are missing the webserver-user as group-member, integrity-check can fix that");
			if ($fix) {
				...
				$this->log->addNotice("Customers were missing the webserver-user as group-member, integrity-check fixed that");
			} else {
				return false;
			}
		}
	...
	public function froxlorLocalGroupMemberForFcgidPhpFpm($fix = false)
	{
		...
		if ($cwg_stmt->rowCount() > 0) {
			$this->log->addNotice("Customers are missing the local froxlor-user as group-member, integrity-check can fix that");
			if ($fix) {
				// prepare update statement
				$upd_stmt = Database::prepare("
	                UPDATE `" . TABLE_FTP_GROUPS . "` SET `members` = CONCAT(`members`, :additionaluser)
				...
				$this->log->addNotice("Customers were missing the local froxlor-user as group-member, integrity-check fixed that");
			} else {
				return false;
			}
		}
