...
class HttpConfigBase
{
	...
	public function init()
	{
		...
		if (Settings::Get('system.leenabled') == 1) {
			$this->logger->addInfo('Running Let\'s Encrypt cronjob prior to regenerating webserver config files');
	...
	public function reload()
	{
		...
		if ((int) Settings::Get('phpfpm.enabled') == 1) {
			...
			foreach ($restart_cmds as $restart_cmd) {
				...
				if ($_conffiles === false || empty($_conffiles)) {
					$this->logger->addInfo($called_class . '::reload: fpm config directory "' . $restart_cmd['config_dir'] . '" is empty. Creating dummy.');
					Fpm::createDummyPool($restart_cmd['config_dir']);
				}
				$this->logger->addInfo($called_class . '::reload: running ' . $restart_cmd['reload_cmd']);
				\Froxlor\FileDir::safe_exec(escapeshellcmd($restart_cmd['reload_cmd']));
			}
		}
		$this->logger->addInfo($called_class . '::reload: reloading ' . $called_class);
		...
		if (Settings::Get('system.webserver') == "nginx" && Settings::Get('system.phpreload_command') != '' && (int) Settings::Get('phpfpm.enabled') == 0) {
			$this->logger->addInfo($called_class . '::reload: restarting php processes');
