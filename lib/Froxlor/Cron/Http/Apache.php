...
class Apache extends HttpConfigBase
{
		...
		if ((int) Settings::Get('phpfpm.enabled') == 1) {
			...
			foreach ($restart_cmds as $restart_cmd) {
				...
				if ($_conffiles === false || empty($_conffiles)) {
					$this->logger->addInfo('apache::reload: fpm config directory "' . $restart_cmd['config_dir'] . '" is empty. Creating dummy.');
					Fpm::createDummyPool($restart_cmd['config_dir']);
				}
				$this->logger->addInfo('apache::reload: running ' . $restart_cmd['reload_cmd']);
				\Froxlor\FileDir::safe_exec(escapeshellcmd($restart_cmd['reload_cmd']));
			}
		}
		$this->logger->addInfo('apache::reload: reloading apache');
	...
	private function createStandardDirectoryEntry()
	{
		...
		$this->virtualhosts_data[$vhosts_filename] .= '  <Directory "' . \Froxlor\FileDir::makeCorrectDir(Settings::Get('system.documentroot_prefix')) . '">' . "\n";
		...
		if (Settings::Get('system.use_ssl') == '1' && Settings::Get('system.apache24') == 1) {
			...
			if (file_exists($ocsp_cache_filename)) {
				$this->logger->addNotice('apache::_createStandardDirectoryEntry: unlinking ' . basename($ocsp_cache_filename));
	...
	public function createIpPort()
	{
		...
		while ($row_ipsandports = $result_ipsandports_stmt->fetch(\PDO::FETCH_ASSOC)) {
			...
			$this->logger->addInfo('apache::createIpPort: creating ip/port settings for  ' . $ipport);
			...
			if ($row_ipsandports['listen_statement'] == '1') {
				...
				$this->logger->addDebug($ipport . ' :: inserted listen-statement');
			}

			if ($row_ipsandports['namevirtualhost_statement'] == '1') {
				// >=apache-2.4 enabled?
				if (Settings::Get('system.apache24') == '1') {
					$this->logger->addInfo($ipport . ' :: namevirtualhost-statement no longer needed for apache-2.4');
				} else {
					$this->virtualhosts_data[$vhosts_filename] .= 'NameVirtualHost ' . $ipport . "\n";
					$this->logger->addDebug($ipport . ' :: inserted namevirtualhost-statement');
				}
			}
			...
			if ($row_ipsandports['vhostcontainer'] == '1') {
				...
				if ($row_ipsandports['ssl'] == '1' && Settings::Get('system.use_ssl') == '1') {
					if ($row_ipsandports['ssl_cert_file'] == '') {
						...
						if (! file_exists($row_ipsandports['ssl_cert_file'])) {
							...
							$this->logger->addDeug('System certificate file "' . Settings::Get('system.ssl_cert_file') . '" does not seem to exist. Disabling SSL-vhost for "' . Settings::Get('system.hostname') . '"');
						}
					}
					...
					if ($domain['ssl_cert_file'] != '') {
						...
						if (! file_exists($domain['ssl_cert_file'])) {
							$this->logger->addError($ipport . ' :: certificate file "' . $domain['ssl_cert_file'] . '" does not exist! Cannot create ssl-directives');
						} else {
							...
							if ($domain['ssl_key_file'] != '') {
								...
								if (! file_exists($domain['ssl_key_file'])) {
									$this->logger->addError($ipport . ' :: certificate key file "' . $domain['ssl_key_file'] . '" does not exist! Cannot create ssl-directives');
								} else {
							...
							if ($domain['ssl_ca_file'] != '') {
								...
								if (! file_exists($domain['ssl_ca_file'])) {
									$this->logger->addError($ipport . ' :: certificate CA file "' . $domain['ssl_ca_file'] . '" does not exist! Cannot create ssl-directives');
								} else {
							...
							if ($domain['ssl_cert_chainfile'] != '') {
								...
								if (! file_exists($domain['ssl_cert_chainfile'])) {
									$this->logger->addError($ipport . ' :: certificate chain file "' . $domain['ssl_cert_chainfile'] . '" does not exist! Cannot create ssl-directives');
								} else {
						...
						$this->logger->addError($domain['domain'] . ' :: empty certificate file! Cannot create ssl-directives');
				...
				$this->logger->addDebug($ipport . ' :: inserted vhostcontainer');
			}
	...
	public function createVirtualHosts()
	{
		...
		foreach ($domains as $domain) {
			...
			$this->logger->addInfo('apache::createVirtualHosts: creating vhost container for domain ' . $domain['id'] . ', customer ' . $domain['loginname']);
		...
		foreach ($diroptions as $row_diroptions) {
			...
			if (is_dir($row_diroptions['path'])) {
				...
				if (isset($row_diroptions['options_indexes']) && $row_diroptions['options_indexes'] == '1') {
					...
					$this->logger->addInfo('Setting Options +Indexes for ' . $row_diroptions['path']);
				}
				...
				if ($cperlenabled && isset($row_diroptions['options_cgi']) && $row_diroptions['options_cgi'] == '1') {
					...
					$this->logger->addInfo('Enabling perl execution for ' . $row_diroptions['path']);
	...
	{
		// Write diroptions
		$this->logger->addInfo("apache::writeConfigs: rebuilding " . Settings::Get('system.apacheconf_diroptions'));
		...
		if (count($this->diroptions_data) > 0) {
			...
			if (! $optsDir->isConfigDir()) {
				...
				if (! file_exists(Settings::Get('system.apacheconf_diroptions'))) {
					$this->logger->addNotice('apache::writeConfigs: mkdir ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir(Settings::Get('system.apacheconf_diroptions'))));
		...
		// Write htpasswds
		$this->logger->addInfo("apache::writeConfigs: rebuilding " . Settings::Get('system.apacheconf_htpasswddir'));
		...
		if (count($this->htpasswds_data) > 0) {
			...
			if ($htpasswdDir->isConfigDir(true)) {
				...
				$this->logger->addWarning('WARNING!!! ' . Settings::Get('system.apacheconf_htpasswddir') . ' is not a directory. htpasswd directory protection is disabled!!!');
			}
		}

		// Write virtualhosts
		$this->logger->addInfo("apache::writeConfigs: rebuilding " . Settings::Get('system.apacheconf_vhost'));
		...
		if (count($this->virtualhosts_data) > 0) {
			...
			if (! $vhostDir->isConfigDir()) {
				...
				if (! file_exists(Settings::Get('system.apacheconf_vhost'))) {
					$this->logger->addNotice('apache::writeConfigs: mkdir ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir(Settings::Get('system.apacheconf_vhost'))));
