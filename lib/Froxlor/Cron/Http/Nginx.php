...
class Nginx extends HttpConfigBase
{
	...
	public function createIpPort()
	{
		...
		while ($row_ipsandports = $result_ipsandports_stmt->fetch(\PDO::FETCH_ASSOC)) {
			...
			$this->logger->addInfo('nginx::createIpPort: creating ip/port settings for  ' . $ip . ":" . $port);
	...
	protected function composeSslSettings($domain_or_ip)
	{
		...
		if ($domain_or_ip['ssl_cert_file'] == '') {
			...
			if (! file_exists($domain_or_ip['ssl_cert_file'])) {
				...
				$this->logger->addDebug('System certificate file "' . Settings::Get('system.ssl_cert_file') . '" does not seem to exist. Disabling SSL-vhost for "' . $domain_or_ip['domain'] . '"');
			}
		}
		...
		if ($domain_or_ip['ssl_cert_file'] != '') {
			...
			if (! file_exists($domain_or_ip['ssl_cert_file'])) {
				$this->logger->addError($domain_or_ip['domain'] . ' :: certificate file "' . $domain_or_ip['ssl_cert_file'] . '" does not exist! Cannot create ssl-directives');
			} else {
				...
				if ($domain_or_ip['ssl_key_file'] != '') {
					...
					if (! file_exists($domain_or_ip['ssl_key_file'])) {
						$this->logger->addError($domain_or_ip['domain'] . ' :: certificate key file "' . $domain_or_ip['ssl_key_file'] . '" does not exist! Cannot create ssl-directives');
					} else {
	...
	{
		$this->logger->addInfo("nginx::writeConfigs: rebuilding " . Settings::Get('system.apacheconf_vhost'));
		...
		if (! $vhostDir->isConfigDir()) {
			...
			if (! file_exists(Settings::Get('system.apacheconf_vhost'))) {
				$this->logger->addNotice('nginx::writeConfigs: mkdir ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir(Settings::Get('system.apacheconf_vhost'))));
				\Froxlor\FileDir::safe_exec('mkdir -p ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir(Settings::Get('system.apacheconf_vhost'))));
			}
