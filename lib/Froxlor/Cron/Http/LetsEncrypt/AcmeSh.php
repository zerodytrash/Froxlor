...
class AcmeSh extends \Froxlor\Cron\FroxlorCron
{
	...
	public static function run($internal = false)
	{
		...
		if (count($issue_domains)) {
			self::$cronlog->addInfo("Requesting " . count($issue_domains) . " new Let's Encrypt certificates");
		...
		if ($changedetected) {
			...
			self::$cronlog->addInfo("No new certificates or certificate updates found");
		}
	}
	...
	private static function runIssueFor($certrows = array())
	{
		...
		foreach ($certrows as $certrow) {
			...
			if ($certrow['ssl_redirect'] != 2) {
				...
				$cronlog->addInfo("Adding common-name: " . $certrow['domain']);
				$domains = array(
					...
					if (! empty($froxlor_aliases)) {
						...
						foreach ($froxlor_aliases as $falias) {
							if (\Froxlor\Validate\Validate::validateDomain(trim($falias))) {
								$cronlog->addInfo("Adding SAN entry: " . strtolower(trim($falias)));
	...
	private static function validateDns(&$domains = array(), $domain_id, &$cronlog)
	{
		if (Settings::Get('system.le_domain_dnscheck') == '1' && ! empty($domains)) {
			...
			foreach ($loop_domains as $idx => $domain) {
				$cronlog->addInfo("Validating DNS of " . $domain);
				// ips accordint to NS
				$domain_ips = PhpHelper::gethostbynamel6($domain);
				if (count(array_intersect($our_ips, $domain_ips)) <= 0) {
					// no common ips...
					$cronlog->addWarning("Skipping Let's Encrypt generation for " . $domain . " due to no system known IP address via DNS check");
					unset($domains[$idx]);
				}
			}
		}
				...
				if (file_exists($ssl_file)) {
					...
					if (! empty($certificate_folder_noecc)) {
						...
						if (file_exists($ssl_file_fb)) {
							$cronlog->addWarning("ECC certificates activated but found only non-ecc file");
							$return[$index] = file_get_contents($ssl_file_fb);
							continue;
						}
					}
					$cronlog->addError("Could not find file '" . $sslfile . "' in '" . $certificate_folder . "'");
					$return[$index] = null;
				}
			}
		} else {
			$cronlog->addError("Could not find certificate-folder '" . $certificate_folder . "'");
		}
	}
	...
	private static function checkInstall($tries = 0)
	{
		if (! file_exists(self::$acmesh) && $tries > 0) {
			self::$cronlog->addError("Download/installation of acme.sh seems to have failed. Re-run cronjob to try again or install manually to '" . self::$acmesh . "'");
