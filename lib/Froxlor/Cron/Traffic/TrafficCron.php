...
class TrafficCron extends \Froxlor\Cron\FroxlorCron
{
	...
	public static function run()
	{
		...
		if (function_exists('pcntl_fork') && ! defined('CRON_NOFORK_FLAG')) {
			...
			if (file_exists($TrafficLock) && is_numeric($TrafficPid = file_get_contents($TrafficLock))) {
				...
				if ($TrafficPidStatus) {
					self::$cronlog->addInfo('Traffic Run already in progress');
					return 1;
				}
			}
			...
			self::$cronlog->addInfo($msg . " Not forking traffic-cron, this may take a long time!");
		}

		/**
		 * TRAFFIC AND DISKUSAGE MESSURE
		 */
		self::$cronlog->addInfo('Traffic run started...');
		$admin_traffic = array();
		$domainlist = array();
		...
		while ($row_database = $databases_stmt->fetch(\PDO::FETCH_ASSOC)) {
			...
			if (in_array(strtolower($row_database['databasename']), $databases_list)) {
				...
				if ($mysql_usage_row) {
					...
					\Froxlor\FroxlorLogger::getLog()->addWarning("Cannot get usage for database " . $row_database['databasename'] . ".");
				}
			} else {
				self::$cronlog->addWarning("Seems like the database " . $row_database['databasename'] . " had been removed manually.");
			}
		}
		...
		while ($row = $result_stmt->fetch(\PDO::FETCH_ASSOC)) {
			 ...
			 * HTTP-Traffic
			 */
			self::$cronlog->addInfo('http traffic for ' . $row['loginname'] . ' started...');
			$httptraffic = 0;
			 ...
			 * FTP-Traffic
			 */
			self::$cronlog->addInfo('ftp traffic for ' . $row['loginname'] . ' started...');
			...
			$mailtraffic = 0;
			if (Settings::Get("system.mailtraffic_enabled")) {
				self::$cronlog->addInfo('mail traffic usage for ' . $row['loginname'] . " started...");
			 ...
			 * Total Traffic
			 */
			self::$cronlog->addInfo('total traffic for ' . $row['loginname'] . ' started');
			 ...
			 * WebSpace-Usage
			 */
			self::$cronlog->addInfo('calculating webspace usage for ' . $row['loginname']);
			$webspaceusage = 0;
			...
			if (Settings::Get('system.diskquota_enabled') && isset($usedquota[$row['guid']]['block']['used']) && $usedquota[$row['guid']]['block']['used'] >= 1) {
				...
				if (file_exists($row['documentroot']) && is_dir($row['documentroot'])) {
					...
					self::$cronlog->addWarning('documentroot ' . $row['documentroot'] . ' does not exist');
				}
			}
			 ...
			 * MailSpace-Usage
			 */
			self::$cronlog->addInfo('calculating mailspace usage for ' . $row['loginname']);
			$emailusage = 0;
			...
			if (file_exists($maildir) && is_dir($maildir)) {
				...
				self::$cronlog->addWarning('maildir ' . $maildir . ' does not exist');
			}

			/**
			 * MySQLSpace-Usage
			 */
			self::$cronlog->addInfo('calculating mysqlspace usage for ' . $row['loginname']);
			$mysqlusage = 0;
	...
	private static function awstatsDoSingleDomain($domain, $outputdir)
	{
		...
		if (file_exists($domainconfig)) {
			...
			if (! file_exists($awbsp)) {
				...
				self::$cronlog->addWarning("Necessary awstats_buildstaticpages.pl script could not be found, no traffic is being calculated and no stats are generated. Please check your AWStats-Path setting");
				exit();
			}

			self::$cronlog->addInfo("Running awstats_buildstaticpages.pl for domain '" . $domain . "' (Output: '" . $staticOutputdir . "')");
			...
			self::$cronlog->addInfo("Gathering traffic information from '" . $file . "'");
	...
	private static function callWebalizerGetTraffic($logfile, $outputdir, $caption, $usersdomainlist)
	{
		...
		if (file_exists($logfile)) {
			...
			self::$cronlog->addInfo("Running webalizer for domain '" . $caption . "'");
			...
			self::$cronlog->addInfo("Gathering traffic information from '" . $webalizer_hist . "'");
			...
			self::$cronlog->addInfo("Gathering traffic information from '" . $webalizer_lasthist . "'");
