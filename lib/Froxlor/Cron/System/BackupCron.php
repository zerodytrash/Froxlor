...
class BackupCron extends \Froxlor\Cron\FroxlorCron
{
	...
	public static function run()
	{
		...
		if (function_exists('pcntl_fork')) {
			...
			if (file_exists($BackupLock) && is_numeric($BackupPid = file_get_contents($BackupLock))) {
				...
				if ($BackupPidStatus) {
					self::$cronjob->addInfo('Backup run already in progress');
					return 1;
				}
			}
			...
			self::$cronjob->addWarning($msg . " Not forking backup-cron, this may take a long time!");
		}

		self::$cronjob->addInfo('BackupCron: started - creating customer backup');
			...
			if (is_array($row['data'])) {
				...
				if (isset($row['data']['customerid']) && isset($row['data']['loginname']) && isset($row['data']['destdir'])) {
					...
					if (! file_exists($row['data']['destdir']) && $row['data']['destdir'] != '/' && $row['data']['destdir'] != Settings::Get('system.documentroot_prefix') && $row['data']['destdir'] != $customerdocroot) {
						self::$cronjob->addNotice('Creating backup-destination path for customer: ' . escapeshellarg($row['data']['destdir']));
	...
	private static function createCustomerBackup($data = null, $customerdocroot = null)
	{
		...
		self::$cronjob->addDebug('Creating tmp-folder "' . $tmpdir . '"');
		self::$cronjob->addDebug('shell> mkdir -p ' . escapeshellarg($tmpdir));
		...
		if ($data['backup_dbs'] == 1) {
			...
			self::$cronjob->addDebug('Creating mysql-folder "' . \Froxlor\FileDir::makeCorrectDir($tmpdir . '/mysql') . '"');
			self::$cronjob->addDebug('shell> mkdir -p ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir($tmpdir . '/mysql')));
			...
			while ($row = $sel_stmt->fetch()) {
				self::$cronjob->addDebug('shell> mysqldump -u ' . escapeshellarg($sql_root['user']) . ' -pXXXXX ' . $row['databasename'] . ' > ' . \Froxlor\FileDir::makeCorrectFile($tmpdir . '/mysql/' . $row['databasename'] . '_' . date('YmdHi', time()) . '.sql'));
				$bool_false = false;
		...
		if ($data['backup_mail'] == 1) {
			...
			self::$cronjob->addDebug('Creating mail-folder "' . \Froxlor\FileDir::makeCorrectDir($tmpdir . '/mail') . '"');
			...
			if (! empty($tar_file_list)) {
				self::$cronjob->addDebug('shell> tar cfvz ' . escapeshellarg(\Froxlor\FileDir::makeCorrectFile($tmpdir . '/mail/' . $data['loginname'] . '-mail.tar.gz')) . ' -C ' . escapeshellarg($mail_homedir) . ' ' . trim($tar_file_list));
		...
		if ($data['backup_web'] == 1) {
			...
			self::$cronjob->addDebug('Creating web-folder "' . \Froxlor\FileDir::makeCorrectDir($tmpdir . '/web') . '"');
			\Froxlor\FileDir::safe_exec('mkdir -p ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir($tmpdir . '/web')));
			self::$cronjob->addDebug('shell> tar cfz ' . escapeshellarg(\Froxlor\FileDir::makeCorrectFile($tmpdir . '/web/' . $data['loginname'] . '-web.tar.gz')) . ' --exclude=' . escapeshellarg(str_replace($customerdocroot, "./", \Froxlor\FileDir::makeCorrectFile($tmpdir . '/*'))) . ' --exclude=' . escapeshellarg(str_replace($customerdocroot, "./", substr(\Froxlor\FileDir::makeCorrectDir($tmpdir), 0, - 1))) . ' -C ' . escapeshellarg($customerdocroot) . ' .');
		...
		if (! empty($create_backup_tar_data)) {
			...
			self::$cronjob->addInfo('Creating backup-file "' . $backup_file . '"');
			// pack all archives in tmp-dir to one
			self::$cronjob->addDebug('shell> tar cfz ' . escapeshellarg($backup_file) . ' -C ' . escapeshellarg($tmpdir) . ' ' . trim($create_backup_tar_data));
			\Froxlor\FileDir::safe_exec('tar cfz ' . escapeshellarg($backup_file) . ' -C ' . escapeshellarg($tmpdir) . ' ' . trim($create_backup_tar_data));
			// move to destination directory
			self::$cronjob->addDebug('shell> mv ' . escapeshellarg($backup_file) . ' ' . escapeshellarg($data['destdir']));
			\Froxlor\FileDir::safe_exec('mv ' . escapeshellarg($backup_file) . ' ' . escapeshellarg($data['destdir']));
			// remove tmp-files
			self::$cronjob->addDebug('shell> rm -rf ' . escapeshellarg($tmpdir));
			\Froxlor\FileDir::safe_exec('rm -rf ' . escapeshellarg($tmpdir));
			// set owner to customer
			self::$cronjob->addDebug('shell> chown -R ' . (int) $data['uid'] . ':' . (int) $data['gid'] . ' ' . escapeshellarg($data['destdir']));
