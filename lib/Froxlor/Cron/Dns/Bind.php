...
class Bind extends DnsBase
{
	...
	public function writeConfigs()
	{
		...
		$this->logger->addInfo('Task4 started - Rebuilding froxlor_bind.conf');
		...
		if (! file_exists(\Froxlor\FileDir::makeCorrectDir(Settings::Get('system.bindconf_directory') . '/domains/'))) {
			$this->logger->addNotice('mkdir ' . escapeshellarg(\Froxlor\FileDir::makeCorrectDir(Settings::Get('system.bindconf_directory') . '/domains/')));
		...
		if (empty($domains)) {
			$this->logger->addInfo('No domains found for nameserver-config, skipping...');
			return;
		}
		...
		$this->logger->addInfo('froxlor_bind.conf written');
		$this->reloadDaemon();
		$this->logger->addInfo('Task4 finished');
	}
	...
	private function walkDomainList($domain, $domains)
	{
		...
		if ($domain['zonefile'] == '') {
			...
			if ($domain['ismainbutsubto'] == 0) {
				...
				$this->logger->addInfo('`' . $zonefile_name . '` written');
			...
			$this->logger->addInfo('Added zonefile ' . $domain['zonefile'] . ' for domain ' . $domain['domain'] . ' - Note that you will also have to handle ALL records for ALL subdomains.');
			$this->bindconf_file .= $this->generateDomainConfig($domain);
		}
	}
	...
	{
		$this->logger->addDebug('Generating dns config for ' . $domain['domain']);
	...
	private function cleanZonefiles()
	{
		...
		$this->logger->addInfo('Cleaning dns zone files from ' . $config_dir);
