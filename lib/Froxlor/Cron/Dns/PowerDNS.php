...
class PowerDNS extends DnsBase
{
	...
	public function writeConfigs()
	{
		...
		$this->logger->addInfo('Task4 started - Refreshing DNS database');
		...
		$this->logger->addInfo('PowerDNS database updated');
		$this->reloadDaemon();
	...
	private function walkDomainList($domain, $domains)
	{
		...
		if ($domain['zonefile'] == '') {
			...
			if ($domain['ismainbutsubto'] == 0) {
				...
				$this->logger->addInfo('DB entries stored for zone `' . $domain['domain'] . '`');
			} else {
				return \Froxlor\Dns\Dns::createDomainZone(($domain['id'] == 'none') ? $domain : $domain['id'], $isFroxlorHostname, true);
			}
		} else {
			$this->logger->addError('Custom zonefiles are NOT supported when PowerDNS is selected as DNS daemon (triggered by: ' . $domain['domain'] . ')');
		}
	}

	private function clearZoneTables($domains = null)
	{
		$this->logger->addInfo('Cleaning dns zone entries from database');
