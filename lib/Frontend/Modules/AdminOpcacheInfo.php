...
class AdminOpcacheInfo extends FeModule
{
	...
	public function reset()
	{
		...
		// check for opcache-stuff
		if (! function_exists('opcache_reset')) {
			\Froxlor\UI\Response::standard_error('no_opcacheinfo');
		}
		opcache_reset();
		\Froxlor\FroxlorLogger::getLog()->addInfo("reseted OPcache");
		\Froxlor\UI\Response::redirectTo("index.php?module=AdminOpcacheInfo");
	}
	...
	public function overview()
	{
		...
		if (isset($opcache_info['directives']) && is_array($opcache_info['directives'])) {
			foreach ($opcache_info['directives'] as $name => $value) {
				...
				if ($value === null || $value === '') {
					$value = \Froxlor\Frontend\UI::getLng('opcacheinfo.novalue');
				}
				if ($value === true) {
					$value = \Froxlor\Frontend\UI::getLng('opcacheinfo.true');
				}
				if ($value === false) {
					$value = \Froxlor\Frontend\UI::getLng('opcacheinfo.false');
				}
				if (is_integer($value)) {
		...
		$general = array(
			...
			'last_restart_time' => @$opcache_status['opcache_statistics']['last_restart_time'] ? date('Y-m-d H:i:s', $opcache_status['opcache_statistics']['last_restart_time']) : \Froxlor\Frontend\UI::getLng('opcacheinfo.never'),
			...
			'status' => (@$opcache_status['restart_in_progress'] ? \Froxlor\Frontend\UI::getLng('opcacheinfo.restartinprogress') : (@$opcache_status['restart_pending'] ? \Froxlor\Frontend\UI::getLng('opcacheinfo.restartpending') : (@$opcache_status['cache_full'] ? \Froxlor\Frontend\UI::getLng('opcacheinfo.cachefull') : (@$opcache_status['opcache_enabled'] ? \Froxlor\Frontend\UI::getLng('opcacheinfo.enabled') : \Froxlor\Frontend\UI::getLng('opcacheinfo.novalue'))))),
